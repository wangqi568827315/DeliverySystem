...
public class DateTime {
	...
	private static int date_date;
	private static int date_week;
	
	private static int time_hour;
	private static int time_min;
	private static int time_sec;
	private static int time_msec;
	
	/**
	 * 这是初始化时间各个部分
	 * */
	static{
		date_year = myCalendar.get(Calendar.YEAR);
		date_month = myCalendar.get(Calendar.MONTH)+1;
		date_date = myCalendar.get(Calendar.DATE);
		date_week = myCalendar.get(Calendar.DAY_OF_WEEK)-1;
		time_hour = myCalendar.get(Calendar.HOUR_OF_DAY);
		time_min = myCalendar.get(Calendar.MINUTE);
		time_sec = myCalendar.get(Calendar.SECOND);
		time_msec = myCalendar.get(Calendar.MILLISECOND);
	}
